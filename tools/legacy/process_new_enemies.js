...
async function processImage(prefix, outName) {
    try {
        ...
        // Only process if it's not already very dark black
        if (rBg > 10 || gBg > 10 || bBg > 10) {
            console.log("Background is not black, processing...");
            image.scan(0, 0, image.bitmap.width, image.bitmap.height, function(x, y, idx) {
                const r = this.bitmap.data[idx + 0];
                const g = this.bitmap.data[idx + 1];
                const b = this.bitmap.data[idx + 2];
                
                // If it's close to the background color
                if (Math.abs(r-rBg) < 15 && Math.abs(g-gBg) < 15 && Math.abs(b-bBg) < 15) {
                    this.setPixelColor(blackColor, x, y);
                }
            });
        } else {
            console.log("Background is already black, keeping as is.");
        }
...
async function run() {
    await processImage('death_knight_', 'death_knight.jpg');
    await processImage('venom_basilisk_', 'venom_basilisk.jpg');
    await processImage('storm_elemental_', 'storm_elemental.jpg');
}
